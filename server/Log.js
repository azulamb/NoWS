...
function CreateOutFileFunction(file, max) {
...
function CreateOutStreamFunction(stream) {
    return (...messages) => {
        const message = messages.map((data) => { return data; }).join(' ') + '\n';
        stream.write(message);
    };
}
function SetFunctions(config) {
    const logs = ['assert', 'debug', 'error', 'info', 'log', 'warn'];
    const funcs = {
        '': () => { },
    ...
    logs.forEach((type) => {
        ...
        if (!funcs[type]) {
            funcs[type] = CreateOutFileFunction(logtype, max);
        }
